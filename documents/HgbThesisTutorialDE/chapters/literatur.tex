...
in Kombination mit dem \latex-Paket \texttt{biblatex} \cite{Kime2026}.
...
(s.\ \url{https://mirrors.ctan.org/macros/latex/contrib/biblatex/doc/biblatex.pdf}).}
%
\begin{itemize}
\item[]
...
Literaturverzeichnis bei Quelle {\backtrackerfalse\parencite{Bezos2026}} 
...
zudem auch auf Englisch umgeschaltet (siehe Abschn.\
...
Dieses Dokument verwendet \texttt{biblatex} in Verbindung mit dem Programm
\texttt{biber}, das viele Unzulänglichkeiten des
...
manuell zu überarbeiten (\sa\ Abschnitt~\ref{sec:TippsZuBiblatex}). Die
Versionen von \texttt{biblatex} und \texttt{biber} müssen übrigens zueinander
passen, was bei einer aktuellen \latex-Distribution automatisch gewährleistet
ist.
...
\documentclass[type=master,language=german,smartquotes=true]{hgbthesis}
   ...
\bibliography{hgbreferences} /+\label{tex:literatur1}+/
   ...
\begin{document}
   ...
...
\end{LaTeXCode}
%
In der "Präambel" (Zeile \ref{tex:literatur1}) wird mit der Anweisung
...
auf eine BibLaTeX-Datei \nolinkurl{hgbreferences.bib} verwiesen, welche alle
...
\begin{itemize}
    ...
    \item[] \textsf{media} -- für Filme, audio-visuelle Medien (auf DVD,
    Streaming \usw);
...
%
	\midrule
	\emph{Audiovisuelle Medien} (\textsf{media}) & & \\
	\midrule
...
BibLaTeX-Eintrag in der Datei \nolinkurl{hgbreferences.bib} angegeben, gefolgt
...
Dieser Publikationstyp bietet sich für jegliche Art von technischer oder anderer
...
\texttt{biblatex}-Handbuch \cite{Kime2026} ansehen, beispielsweise
...
\texttt{type} spezifiziert werden, um welche Art von Dokument es sich handelt.
...
\cite[Abschn.\ 4.9.2.11]{Kime2026}. Andernfalls (wenn unbekannt oder wirklich
...
\subsection{Filme und audio-visuelle Medien (\textsf{media})}
...
Einen Verweis auf eine bestimmte Stelle in einem Musikstück oder Film kann
...
Bei Verweisen auf Online-Ressourcen sind grundsätzlich drei Fälle zu
unterscheiden:
%
\begin{itemize}
    \item[A.] Man möchte allgemein auf eine Webseite verweisen, etwa auf die
    Website des Softwareparks Hagenberg.%
...
Hauptdatei die Option \texttt{apa=true} angegeben werden, \zB
%
\begin{LaTeXCode}[numbers=none]
\documentclass[type=master,language=german,smartquotes=true,apa=true]{hgbthesis}
\end{LaTeXCode}
%
...
\begin{itemize}
    ...
    gekennzeichnet sein!
    \item
    Werden Texte mithilfe von KI-Tools erzeugt oder überarbeitet, ist das in
    der Dokumentation der Nutzung von KI-Tools anzugeben (s.\
    Abschnitt~\ref{sec:KIDokumentation}). Mit KI-Tools generierte Inhalte
    gelten nicht als eigene Leistung; mit der Eidesstattlichen Erklärung wird
    zudem bestätigt, dass die Arbeit selbständig verfasst und nur die
