...
weitere dazu findet sich \zB in \cite[Kap.\ 7]{Kopka2003} und im
\citetitle{Downes2017} \cite{Downes2017}.
...
Im Fließtext wird bei einfachen Variablen oft vergessen, die richtigen
mathematischen Zeichen zu verwenden, wie etwa in "X-Achse" anstelle von
"$X$-Achse" (\verb!$X$-Achse!).


\subsubsection{Schriften im Mathematik-Modus}

\latex\ verwendet leicht unterschiedliche Schriftarten für normalen Text und im
...
\begin{quote}
    \begin{tabular}{lcl}
        $\mathrm{Roman}$      & & \verb!$\mathrm{Roman}$!,      \\
				...
				$\mathbf{Bold}$     	& & \verb!$\mathbf{Bold}$!,      \\
        $\mathsf{Sans Serif}$ & & \verb!$\mathsf{Sans Serif}$!, \\
        $\mathtt{Typewriter}$ & & \verb!$\mathtt{Typewriter}$!, \\
...
\section{Freigestellte Ausdrücke und Gleichungen}
...
keine Nummerierung. So ist \zB\ \[y_0 = 4 x^2\] das Ergebnis von
\verb!\[y_0 = 4 x^2\]! oder alternativ
%
...
\subsection{Einzelne nummerierte Gleichungen}
...
\begin{center}
	...
	\fbox{%
		...
		Man beachte, dass \emph{Gleichungen} inhaltlich ein \emph{Teil
		des Texts} sind und daher neben der sprachlichen
		\emph{Überleitung} auch die \emph{Interpunktion} (wie in Gl.\
...
\latex\ Standardinstallation und wird von \texttt{hgbmath.sty} bereits importiert.}
\cite{Mittelbach2025} zurückzugreifen. Hier ein Beispiel mit zwei am $=$
...
\cite{Mittelbach2025} beschrieben, weitere Ergänzungen dazu bietet das Paket
\texttt{mathtools}.%
\footnote{\url{https://ctan.org/pkg/mathtools}}
...
wenigen mathematischen Elementen -- "Gleichung \ref{eq:f1}",
"Gl.~\ref{eq:f1}" oder "Gl.~(\ref{eq:f1})" geschrieben werden (natürlich
konsistent).
%
\begin{center}
	\setlength{\fboxrule}{0.2mm}
	...
	\fbox{%
		\begin{minipage}{0.9\textwidth}
			Achtung: Vorwärtsverweise auf (im Text weiter hinten
			liegende) Gleichungen sind \emph{äußerst ungewöhnlich}
...
Makros%
\footnote{Mithilfe der \emph{AMS Blackboard Fonts}.}
\verb!\R!, \verb!\Z!, \verb!\N!, \verb!\Cpx!, \verb!\Q! ($\R, \Z, \N,
\Cpx, \Q$) definiert, \zB:
%
\begin{center}
...
\end{quote}
%
Komfortabler ist das Paket \texttt{icomma},%
\footnote{\url{https://ctan.org/pkg/icomma}}
mit dem ein Komma, auf das unmittelbar eine Ziffer folgt, als
Dezimaltrennzeichen gesetzt wird, während ein Komma mit nachfolgendem
Leerzeichen weiterhin als Satzzeichen gilt. Für die einheitliche Formatierung
von Zahlen und Maßeinheiten (einschließlich des Dezimaltrennzeichens)
empfiehlt sich zudem das Paket \texttt{siunitx}.%
\footnote{\url{https://ctan.org/pkg/siunitx}}
Beide Pakete sind nicht Teil dieser Vorlage und müssen bei Bedarf in der
Präambel des Dokuments geladen werden.


\subsection{Mathematische Werkzeuge}

Für die Erstellung komplizierter Gleichungen ist es mitunter hilfreich, auf
spezielle Software zurückzugreifen. So kann etwa der Formeleditor von
Microsoft \emph{Word} Gleichungen im \latex-Format ein- und ausgeben, und aus
\emph{Mathematica} lassen sich \latex-An\-wei\-sun\-gen exportieren. Werkzeuge
wie \emph{Mathpix} erkennen Formeln in Bildern oder PDF-Dokumenten, und
\emph{Detexify}%
\footnote{\url{https://detexify.kirelabs.org/classify.html}}
hilft beim Auffinden des passenden \latex-Makros für ein von Hand gezeichnetes
Symbol. Die Ergebnisse können (meist mit etwas manueller Nacharbeit) direkt in
das eigene \latex-Dokument übernommen werden.
...
\footnote{Siehe auch
\url{https://mirrors.ctan.org/macros/latex/contrib/algorithms/algorithms.pdf}
(Abschnitt~7).}
%
\begin{itemize}
	\item
	Verwende in Algorithmen die gleichen kurzen Symbole (wie $a, i, x, S,
