...
Stan\-dard-Pakets \texttt{graphicx} \cite{Carlisle2026} empfohlen (wird durch
...
Grundsätzlich sollten auch die in Grafiken verwendeten Schriften möglichst genau
mit der Schrift im Haupttext übereinstimmen. Ein interessantes Werkzeug dafür
...
\emph{Garamond}) verwenden. Es ist aber auch möglich, die in diesem Dokument
verwendeten \emph{Latin-Modern}-Schriften, eine Weiterentwicklung der
\emph{Computer-Modern}-Schriften (CM) von {\tex}/{\latex}, direkt zur Erzeugung
von Grafiken einzusetzen. Sie stehen auch in Formaten zur Verfügung, die in
herkömmlichen Grafikprogrammen unter \emph{Windows}, \emph{macOS} und
\emph{Linux} verwendet werden können:
%
\begin{itemize}
\item	%\subsubsection{\emph{Latin Modern} Fonts (OpenType)}
Empfehlenswert sind die "Latin Modern" OpenType-Schriften,%
\footnote{\url{https://www.gust.org.pl/projects/e-foundry/latin-modern}}
die speziell für die Verwendung im Umfeld von \latex\ entwickelt wurden und
auch Teil gängiger \latex-Distributionen sind (\zB im Verzeichnis
\nolinkurl{fonts/opentype/public/lm}). Diese Schriften enthalten \ua\ Zeichen
mit Umlauten und sind daher auch für deutsche Texte gut geeignet.
\item	%\subsubsection{\emph{BaKoMa}-Schriften (TrueType)}
Eine Alternative dazu ist die "BaKoMa Fonts Collection" (TrueType),%
\footnote{\url{https://ctan.org/pkg/bakoma-fonts}}
die neben den CM-Standardschriften auch die mathematischen Schriften der
AMS-Familie ent\-hält.
\end{itemize}
%
Natürlich müssen diese Schriften vor der Verwendung zunächst auf dem eigenen
Rechner installiert werden.

\subsection{Grafiken mit \latex-Overlays (\texttt{overpic})}
\label{sec:GraphicOverlays}

Manchmal ist es erforderlich, ein bestehendes Bild oder eine Grafik mit
...
\begin{table}
	...
	\begin{tabular}{@{}llll@{}}
		...
		\midrule
		C++        & Kompiliert    & Applikationen    & ISO/IEC 14882:2024 \\
		COBOL      & Kompiliert    & Business         & ISO/IEC 1989:2023  \\
...
\begin{table}
	...
	\begin{english}
		\begin{tabular}{@{}p{0.2\textwidth}lp{0.3\textwidth}p{0.2\textwidth}@{}}
			...
			{\RR depth values} &
			\\
			{\RR Gouraud shading with z-buffer} &
			SW/HW &
...
\footnote{%
\url{https://mirrors.ctan.org/macros/latex/required/tools/longtable.pdf}}
...
%
\begin{table}
\caption{In \nolinkurl{hgblistings.sty} vordefinierte Code-Umgebungen.}
\label{tab:CodeUmgebungen}
\centering
\begin{tabular}{@{}lll@{}}
	\toprule
	Bash:        & \verb!\begin{BashCode}!
		& \verb!...! \verb!\end{BashCode}! \\
	C:           & \verb!\begin{CCode}!
		& \verb!...! \verb!\end{CCode}! \\
	C++:         & \verb!\begin{CppCode}!
		& \verb!...! \verb!\end{CppCode}! \\
	C\#:         & \verb!\begin{CsCode}!
		& \verb!...! \verb!\end{CsCode}! \\
	CSS:         & \verb!\begin{CssCode}!
		& \verb!...! \verb!\end{CssCode}! \\
	Dart:        & \verb!\begin{DartCode}!
		& \verb!...! \verb!\end{DartCode}! \\
	GLSL:        & \verb!\begin{GlslCode}!
		& \verb!...! \verb!\end{GlslCode}! \\
	Go:          & \verb!\begin{GoCode}!
		& \verb!...! \verb!\end{GoCode}! \\
	...
	JSON:        & \verb!\begin{JsonCode}!
		& \verb!...! \verb!\end{JsonCode}! \\
	Kotlin:      & \verb!\begin{KotlinCode}!
		& \verb!...! \verb!\end{KotlinCode}! \\
	\latex:      & \verb!\begin{LaTeXCode}!
		& \verb!...! \verb!\end{LaTeXCode}! \\
	MATLAB:      & \verb!\begin{MatlabCode}!
		& \verb!...! \verb!\end{MatlabCode}! \\
	...
	R:           & \verb!\begin{RCode}!
		& \verb!...! \verb!\end{RCode}! \\
	Rust:        & \verb!\begin{RustCode}!
		& \verb!...! \verb!\end{RustCode}! \\
	Scala:       & \verb!\begin{ScalaCode}!
		& \verb!...! \verb!\end{ScalaCode}! \\
	SQL:         & \verb!\begin{SqlCode}!
		& \verb!...! \verb!\end{SqlCode}! \\
	Swift:       & \verb!\begin{SwiftCode}!
		& \verb!...! \verb!\end{SwiftCode}! \\
	TypeScript:  & \verb!\begin{TsCode}!
		& \verb!...! \verb!\end{TsCode}! \\
	XML:         & \verb!\begin{XmlCode}!
		& \verb!...! \verb!\end{XmlCode}! \\
	YAML:        & \verb!\begin{YamlCode}!
		& \verb!...! \verb!\end{YamlCode}! \\
...
\ref{MathInCode} von Prog.~\ref{prog:CodeExample}) stehen kann: Mit der Option
\verb![mathescape=true]! wird Text zwischen \verb!$! \ldots\ \verb!$! als
Mathematik gesetzt und fügt sich dabei in die Darstellung des Kommentars ein.


\subsubsection{Darstellung des Codes}
\label{sec:CodeStil}

Programmcode wird standardmäßig mit Syntaxhervorhebung gesetzt:
Schlüsselwörter, eingebaute Typen und Funktionen, Strings und Kommentare
erscheinen in Farben, die jenen gängiger Entwicklungsumgebungen (\zB\ Visual
Studio Code) ähneln und ausreichend Kontrast bieten, neben einer dünnen grauen
Linie mit den Zeilennummern. Mit der Dokumentoption \texttt{codestyle} kann für
das gesamte Dokument eine andere Darstellung gewählt werden:
%
\begin{itemize}
	\item[] \texttt{codestyle=color} (\emph{default}): farbige Syntaxhervorhebung,
	\item[] \texttt{codestyle=mono}: dasselbe Layout ohne Farben, \zB\ für den
		Schwarz-Weiß-Druck,
	\item[] \texttt{codestyle=classic}: grauer Hintergrund ohne Farben (die
		Darstellung früherer Versionen dieses Pakets).
\end{itemize}
%
Die Farben, die Verwendung der Stile für einzelne Listings sowie die
Sprachnamen für das Einbinden von Code-Dateien mit \verb!\lstinputlisting!
sind im Manual des Pakets beschrieben.
...
der Zeilennummerierung hilfreich sind. Im Normalfall (also ohne zusätzliche
Angabe) mit
%
\begin{LaTeXCode}[numbers=none]
...
\begin{JavaCode}[mathescape=true]
import ij.ImagePlus;
