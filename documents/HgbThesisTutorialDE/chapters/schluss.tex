...
mit Bedacht angenommen werden. Ihr Einsatz ist außerdem in der Dokumentation
der Nutzung von KI-Tools anzugeben (siehe Abschnitt~\ref{sec:KIDokumentation}).
...
\begin{table}
	\caption{Liste mit wichtigen Punkten, wie sie typischerweise auch
	im Rahmen einer akademischen \emph{Formbegutachtung} überprüft werden.}
	...
	\fbox{%
		...
		\begin{itemize}
			...
			\item[$\Box$] \textbf{Eidesstattliche Erklärung:}
				vollständige Unterschrift.
			\item[$\Box$] \textbf{Dokumentation der Nutzung von KI-Tools:}
				zutreffende Textbausteine, vollständige Angaben.
			...
			\item[$\Box$] \textbf{Layout/Typographie:}
				sauberer Ausdruck (keine gerasterten Schriften), keine "manuellen"
				Abstände zwischen Absätzen oder Einrückungen, keine überlangen
				Zeilen, Hervorhebungen, Schriftgröße, Platzierung von Fußnoten.
			...
			\item[$\Box$] \textbf{Quellenangaben und Querverweise:}
				Zitate richtig referenziert, inklusive Seiten- und Kapitelangaben;
				keine ungültigen Querverweise (\textbf{??}).
			...
			\item[$\Box$] \textbf{Sonstiges:}
				Inhalte im Anhang, 
				Papiergröße der PDF-Datei (A4 = $8.27 \times 11.69$ Zoll), Druckgröße
				und -qualität.
		\end{itemize}
		\end{minipage}%
	}
