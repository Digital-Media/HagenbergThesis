...
Any theme style must (re)define the \verb!\hgb@MakeFrontPages! command, which 
is invoked by \verb!\maketitle!. 
This command is predefined by the \verb!hgbtheme-thesis! theme template, but 
may be redefined by inheriting themes if needed, \eg:
%
\begin{LaTeXCode}[numbers=none]
\usepackage{hgbtheme-thesis}%  /+ parent theme (in 'hgbtheme-thesis.sty')+/
...
\renewcommand{\hgb@MakeFrontPages}{...}
\end{LaTeXCode}
%
In addition, the template theme \texttt{hgbtheme-thesis.sty} defines the following
...
\medskip
  \item \verb!\hgb@StrictLicense!
  \item \verb!\hgb@ccLicense!
  \item \verb!\hgb@TheLicense!
\end{itemize}
%
\noindent
The \verb!\maketitle! command also invokes the command
\begin{itemize}
  \item \verb!\hgb@InitTheme!
\end{itemize}
(with no parameters), which initially does nothing but may be redefined inside theme styles, \eg,
to initialize certain variables.%
\footnote{This may be replaced by a call hook in the future.}
