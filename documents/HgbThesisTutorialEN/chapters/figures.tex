...
\begin{figure}
    ...
    \caption{Coca-Cola advertisement, 1940 \cite{CocaCola1940}.}
    \label{fig:CocaCola}
\end{figure}
...
standard package \texttt{graphicx} \cite{Carlisle2026} is recommended
...
For illustrations and schematic diagrams (\eg, flowcharts, entity-relationship
...
\emph{Garamond}). However, it is also possible to use the \emph{Latin Modern}
fonts of this document, an extension of the \emph{Computer Modern} (CM) fonts
of {\tex}/{\latex}, directly to generate graphics. They are also available in
formats that can be used in conventional graphics programs under
\emph{Windows}, \emph{macOS}, and \emph{Linux}:
%
\begin{itemize}
    \item
    Recommendable are the "Latin Modern" OpenType fonts,%
    \footnote{\url{https://www.gust.org.pl/projects/e-foundry/latin-modern}}
    which were specifically developed for use in the \latex environment and
    are also part of common \latex distributions (\eg, in the directory
    \nolinkurl{fonts/opentype/public/lm}). These fonts include dedicated glyphs
    for "umlauts" and are therefore well suited for German texts as well.
    \item
    An alternative is the "BaKoMa Fonts Collection" (TrueType),%
    \footnote{\url{https://ctan.org/pkg/bakoma-fonts}}
    which contains (besides the CM standard fonts) also the mathematical fonts
    of the AMS family.
\end{itemize}
%
...
\subsection{Graphics With \latex Overlays (Using \texttt{overpic})}
...
accomplished using the \texttt{overpic} package,%
\footnote{\url{https://ctan.org/pkg/overpic}}
where the underlying graphic
...
\begin{figure}
    ...
    DALL$\cdot$E~3 using the prompt "Create a photo-realistic image of the
...
\begin{table}
    ...
    \begin{tabular}{@{}llll@{}}
        ...
        \midrule
        C++        & Compiled    & Applications     & ISO/IEC 14882:2024 \\
        COBOL      & Compiled    & Business         & ISO/IEC 1989:2023  \\
...
number of columns---preferably left-aligned (\texttt{l}
specifiers)---is defined first. \verb|\toprule| marks the beginning of the
...
also be placed inside comments: with the option \verb![mathescape=true]!, text
between \verb!$! \ldots\ \verb!$! is typeset as math and fits into the
appearance of the comment.


\subsubsection{Code Style}
\label{sec:CodeStyle}

By default, program code is typeset with syntax highlighting: keywords,
built-in types and functions, strings and comments are shown in colors similar
to common development environments (\eg, Visual Studio Code) with sufficient
contrast, next to a thin gray line with the line numbers. The class option
\texttt{codestyle} selects a different style for the whole document:
%
\begin{itemize}
	\item[] \texttt{codestyle=color} (\emph{default}): syntax highlighting in color,
	\item[] \texttt{codestyle=mono}: the same layout without colors, \eg, for
		printing in black and white,
	\item[] \texttt{codestyle=classic}: gray background without colors (the look
		of earlier versions of this package).
\end{itemize}
%
The colors, the use of the styles for individual listings and the language
names for including code files with \verb!\lstinputlisting! are described in
the package manual.
