...
\footnote{"What You See Is What You Get." Real \textsc{WYSIWYG} editors for
\latex hardly exist. However, a similar way of working is offered, for example,
by the visual editor of \textsf{Overleaf} or by \textsf{LyX}, which follows the
\textsc{WYSIWYM} principle ("What You See Is What You Mean").}
...
\end{itemize}
%
can be recommended. The most comprehensive and up-to-date reference work is
\citetitle{Mittelbach2023} by \textcite{Mittelbach2023}. Numerous other
documents on \latex and related topics can be
...
\end{quote}
%
Particularly useful are \citetitle{Pakin2026} \cite{Pakin2026} and the
...
This template uses the \emph{Latin Modern} fonts derived from them (package
\texttt{lmodern}) with the T1 font encoding (package \texttt{fontenc}), which,
among other things, contain accented characters such as umlauts as separate
glyphs and thus allow correct hyphenation and searchable PDF documents. These
fonts are loaded regardless of whether PDF/A output (\texttt{hgbpdfa}, see
Section~\ref{sec:PDFA}) is produced. The three basic font types are
%
\begin{quote}
    \begin{tabular}{lcl}
...
a reason to use \latex for professional purposes. Another advantage of these
fonts is that the different font families and weights are very well-matched in
size.

In addition, other fonts can be used in \latex. For \texttt{pdflatex},
numerous ready-made font packages are available (\eg, for \emph{Times} or
\emph{Palatino}); with \texttt{lualatex} and the \texttt{fontspec} package, any
installed \emph{OpenType} font can be used directly. However, changing the
fonts should be well considered, since it changes the overall appearance of the
thesis.
...
\begin{center}
	...
	\fbox{%
		\begin{minipage}{0.9\textwidth}
			...
			\texttt{\textbackslash{newline}} commands to separate paragraphs is a
...
tables because of the small column width. Using ragged-right alignment (\eg, in
...
%
\begin{LaTeXCode}[numbers=none]
\ref{/+\emph{key}+/} /+\quad or \quad+/ \pageref{/+\emph{key}+/}
\end{LaTeXCode}
%
...
(compared to the usual space between words) after the end of each sentence.
...
\begin{itemize}
  ...
  \item \nolinkurl{hgbtheme-*.sty} (style files): themes for the design of the
    title pages, selected with the class option \texttt{theme} (\texttt{default},
    \texttt{fhooe26}, \texttt{classic}, or \texttt{fhooe24}; see
    Section~\ref{sec:hagenberg-settings}).
  \item Additional style files imported by \nolinkurl{hgbthesis.cls}:
    \begin{itemize}
        \item[] \nolinkurl{hgbabbrev.sty} (various abbreviations),
        \item[] \nolinkurl{hgbaiusage.sty} (documentation of AI tools usage; the
          texts of the detailed form in \nolinkurl{hgbaiusage-cognisance.sty} are
          licensed under CC BY-SA 4.0),
        \item[] \nolinkurl{hgbalgo.sty} (algorithms),
        \item[] \nolinkurl{hgbbib.sty} (reference management),
        \item[] \nolinkurl{hgbdict.sty} (language-dependent terms),
        ...
        \item[] \nolinkurl{hgbvars.sty} (shared variables and options).
    \end{itemize}
\end{itemize}
...
%
\begin{itemize}
    \item[] \texttt{master} (master's thesis = \emph{default}),
    \item[] \texttt{diploma} (diploma thesis),
    \item[] \texttt{bachelor} (bachelor's thesis),
    \item[] \texttt{phd} (doctoral thesis),
...
%
\begin{itemize}
		\item[] \texttt{theme=\emph{theme}} (design of the title page: \texttt{default}
			(\emph{default}), \texttt{fhooe26}, \texttt{classic}, or \texttt{fhooe24};
			details in the package manual).
		\item[] \texttt{titlelanguage=\emph{language}} (language of the title pages,
			defaults to the main language).
		\item[] \texttt{degree=\emph{key}} (academic degree on the title page, \eg,
			\texttt{engineering} for ``Bachelor/Master of Science in Engineering''; all
			supported values are listed in the package manual).
		\item[] \texttt{proposal=true} (thesis proposal, in connection with 
			\texttt{type=bachelor} or \texttt{master}).
    \item[] \texttt{smartquotes=true} (use of upright double quotes, see
			Section~\ref{sec:quotation-marks}).
		\item[] \texttt{twoside=false} (\emph{default}) or \texttt{twoside=true} for 
			a one- or two-sided document layout, respectively.
		\item[] \texttt{apa=true} (APA citation style instead of the numeric style,
			see Section~\ref{sec:APA}).
		\item[] \texttt{codestyle=color} (\emph{default}), \texttt{mono}, or \texttt{classic}
			(appearance of program code, see Section~\ref{sec:CodeStyle}).
		\item[] \texttt{updatecheck=false} (suppresses the warning if the package
			version used is outdated; \emph{default}: \texttt{true}).
\end{itemize}
%
If \emph{none} of the above options is specified, the default settings 
\begin{itemize}
...
\begin{itemize}
    ...
    \verb!\advisor[!\texttt{\em Role}\verb!]{!\texttt{\em Name}\verb!}! --
    up to four advisors, the role is optional.
\end{itemize}
%
\noindent
...
\begin{itemize}
    \item[] %
    \verb!\subtitle{!\texttt{\em Subtitle}\verb!}! --
    only if absolutely necessary, \newline%
    \verb!\institution{!\texttt{\em Institution}\verb!}! --
    default: provided by the title page theme, \newline%
    \verb!\programtype{!\texttt{\em Type of program}\verb!}! --
    default: derived from \texttt{type}, \newline%
    \verb!\academicdegree{!\texttt{\em Academic degree}\verb!}! --
    free text instead of the option \texttt{degree}.
\end{itemize}
%
\noindent
...
\begin{table}
    \caption{Elements in title pages for different document options
        ($+$ \ldots\ required, $\circ$ \ldots\ optional, $-$ \ldots\ not used).}
...
\begin{center}
    \begin{tabular}{@{}cl@{}}
        ...
        \textrm{i}   & Copyright page \\
        \textrm{ii}  & Affidavit \\
        \bottomrule
    \end{tabular}
\end{center}
%
The page numbers apply to the single-sided layout. In internship reports, the
title page is followed by an additional page with details of the internship.
The documentation of AI tools usage is included directly after
\verb!\maketitle! (see Section~\ref{sec:DocumentingAIUsage}).
The copyright page%
...
\begin{description}
    ...
    If necessary, this can specify an alternative \texttt{\emph{license text}}.
...
\texttt{hagenberg-thesis} package. All packages used are part of common \latex\
distributions (\eg, TeX Live or MiKTeX) and are also available in
\textsf{Overleaf}. The corresponding documentation (as PDF) can be opened
locally with \texttt{texdoc \emph{packagename}} or found online on the CTAN
sites given in Section~\ref{sec:literature}.
...
\begin{table}
    ...
    \latex\ standard installations (\eg, TeX Live, MiKTeX).}
    \label{tab:packages}
    \centering\small
    \begin{tabular}{@{}ll@{}}
        \toprule
        ...
        \midrule
        \texttt{algpseudocodex}, \texttt{algorithm} & Description of algorithms \\
        \texttt{amsmath}, \texttt{amsfonts},
        \texttt{amssymb}, \texttt{amsbsy}             & Mathematical typesetting and symbols \\
        \texttt{babel}                                & Language switching \\
        \texttt{biblatex} (with \texttt{biber})       & Bibliography management \\
        \texttt{booktabs}                             & Tables with a professional layout \\
        \texttt{caption}                              & More flexible captions \\
        \texttt{csquotes}                             & Context-sensitive quotation marks \\
        \texttt{datetime2}                            & Dates (\eg, date of submission) \\
        \texttt{enumitem}                             & Customizing lists \\
        \texttt{exscale}                              & Correct font sizes in math mode \\
        \texttt{fancyhdr}, \texttt{extramarks}        & Controlling headers and footers \\
        \texttt{float}                                & Improved float handling \\
        \texttt{fontenc}, \texttt{lmodern}            & T1 font encoding, Latin Modern fonts \\
        \texttt{geometry}                             & Page layout (type area) \\
        \texttt{graphicx}                             & Including graphics \\
        \texttt{hyperref}, \texttt{hypcap}            & Handling of cross-references in the PDF document \\
        \texttt{inputenc}                             & Extended input encodings \\
        \texttt{listings}, \texttt{listingsutf8}      & Code listings \\
        \texttt{longtable}, \texttt{multirow}         & Multipage tables, joined rows \\
        \texttt{marvosym}                             & \euro symbol (\verb!\euro!) \\
        \texttt{overpic}, \texttt{pict2e}             & Graphics with \latex overlays \\
        \texttt{pdfmanagement}                        & PDF/A output (with \texttt{hgbpdfa}) \\
        \texttt{pdfpages}                             & Including PDF documents \\
        \texttt{textpos}                              & Positioning on the title page \\
        \texttt{titlesec}, \texttt{tocbasic}          & Design of headings and lists (\eg, contents) \\
...
references, and similar issues. In any case, the log should eventually contain no
errors and, if possible, no warnings. Most editors (\eg, \textsf{TeXstudio}) as
well as \textsf{Overleaf} list errors, warnings, and messages about overfull or
underfull lines (\emph{overfull} or \emph{underfull boxes}) clearly in a
separate view, from which one can jump directly to the corresponding location
in the source text. 
