...
\cite[Ch.\ 7]{Kopka2003} and in the \citetitle{Downes2017} \cite{Downes2017}.
...
%
\begin{itemize}
	\item[a)] For example, a simple row vector of length $n$ is defined in the form
	$\boldsymbol{x} = (x_0, x_1, \ldots, x_{n-1})$.
	\item[b)] For example, a simple row vector of length $n$ is defined in the form
...
\latex\ default installation and is automatically imported by \texttt{hgbmath.sty}.}
\cite{Mittelbach2025} for this right away. Here is an example with two equations aligned to the $=$ sign,
%
\begin{align}
...
described in \cite{Mittelbach2025}; further extensions are provided by the
\texttt{mathtools} package.%
...
mathematical character set of \latex, such as the symbols for the real and natural 
...
When specifying units of measurement, normal font (no italics) is usually
used, \eg:
%
\begin{quote}
...
\emph{dot} (\verb!.!) is used as the decimal separator.
...
\end{quote}
%
A more convenient solution is the \texttt{icomma} package,%
\footnote{\url{https://ctan.org/pkg/icomma}}
which typesets a comma that is directly followed by a digit as a decimal
separator, while a comma followed by a space is still treated as punctuation.
For the consistent formatting of numbers and units of measurement (including
the decimal separator), the \texttt{siunitx} package is recommended as well.%
\footnote{\url{https://ctan.org/pkg/siunitx}}
Both packages are not part of this template and must be loaded in the preamble
...
to special software or interactive tools. For example, the equation editor of
Microsoft \emph{Word} can read and write equations in \latex format, and
\latex statements can be exported from \emph{Mathematica}. Tools such as
\emph{Mathpix} recognize formulas in images or PDF documents, and
\emph{Detexify}%
\footnote{\url{https://detexify.kirelabs.org/classify.html}}
helps to find the right \latex command for a hand-drawn symbol. The results can
be copied directly (usually with some manual rework) into your own \latex
document.
