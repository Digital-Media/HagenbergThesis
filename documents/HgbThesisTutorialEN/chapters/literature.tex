...
in combination with the \latex\ package \texttt{biblatex} \cite{Kime2026}.
...
(see \url{https://mirrors.ctan.org/macros/latex/contrib/biblatex/doc/biblatex.pdf}).}
%
\begin{itemize}
\item[]
...
\end{itemize}
%
where an associated \textit{text} can be specified for each citation key
(\textit{key}), for example:
%
\begin{itemize}
...
{\backtrackerfalse\parencite{Bezos2026}} of the bibliography 
...
referenced in a more recent work from 1969 (\textrm{B}). One could accomplish this,
...
\footnote{\url{https://mirrors.ctan.org/biblio/biber/base/documentation/biber.pdf}}
...
This document uses \texttt{biblatex} in conjunction with the software
\texttt{biber}, which addresses many shortcomings of the
...
Section~\ref{sec:tips-on-biblatex}). By the way, the versions of
\texttt{biblatex} and \texttt{biber} must match, which is automatically the
case with a current \latex distribution.
...
\documentclass[type=master,language=english,smartquotes=true]{hgbthesis}
   ...
\bibliography{hgbreferences} /+\label{tex:references1}+/
   ...
\begin{document}
   ...
...
%
In the "preamble",
\verb!\bibliography{hgbreferences}!%
...
(line \ref{tex:references1}) refers to a BibLaTeX file \nolinkurl{hgbreferences.bib},
which holds all references.
...
\begin{itemize}
    ...
    in print or online;
    \item[] \textsf{media} -- for movies, audio-visual media (on DVD,
    streaming, \etc);
...
%
	\midrule
	\emph{Audiovisual media} (\textsf{media}) & & \\
	\midrule
...
\begin{tabular}{@{}llll@{}}
	\toprule
	\textsf{literature} & \textsf{media} & \textsf{software} & \textsf{online} \\
	\midrule
...
@thesis{Loimayr2019,
  author={Loimayr, Nora},
  title={Utilization of GPU-Based Smoothed Particle Hydrodynamics for Immersive Audiovisual Experiences},
  type={mathesis},
  date={2019-11-26},
...
\paragraph{Bachelor's Thesis:} ~ \newline
Bachelor's theses are not usually considered "proper" publications, but it must
...
\end{GenericCode}
\item[\cite{Mittelbach2025}] \fullcite{Mittelbach2025}
\end{itemize}
%
...
@legislation{OoeRaumordnungsgesetz1994,
  ...
  howpublished={LGBl.Nr. 114/1993 zuletzt geändert durch LGBl.Nr. 62/2026},
  date={1993-12-23},
...
\texttt{biblatex} manual \cite{Kime2026}. For example, \texttt{@collection} for
...
\cite[Sec.\ 4.9.2.11]{Kime2026}. Otherwise (if unknown or not published),
%
\begin{itemize}
\item[]\texttt{note=\{unpublished\}} \quad or
   \quad \texttt{note=\{unveröffentlicht\}}
\end{itemize}
%
can be used instead of \texttt{pubstate}.


\subsection{Movies and Audio-Visual Media (\textsf{media})}
...
@movie{Nosferatu1922,
  ...
  howpublished={Film},
  date={1922},
  note={Directed by F.\ W.\ Murnau, screenplay by Henrik Galeen. Starring Max Schreck, Gustav von Wangenheim, Greta Schröder.},
  langid={english}
}
\end{GenericCode}
...
%
\begin{itemize}
    \item[A.] Referring to a web page in general, such as the website of the
    Softwarepark Hagenberg.%
    \footnote{\url{https://www.softwarepark-hagenberg.at/}}
...
\end{GenericCode}
\item[\cite{WikiReliquienschrein2023}] \begin{sloppypar}\fullcite{WikiReliquienschrein2023}\end{sloppypar}
\end{itemize}
%
...
of a conference), and \texttt{urldate} (access date of a URL).
...
\begin{itemize}
    ...
    \item For technical reports, master's theses, and dissertations, the
...
To use APA as the citation style throughout the document, the option
\texttt{apa=true} must be specified in the main file, \eg,
%
\begin{LaTeXCode}[numbers=none]
\documentclass[type=master,language=english,smartquotes=true,apa=true]{hgbthesis}
\end{LaTeXCode}
%
...
\textrm{\url{https://mirrors.ctan.org/info/lshort/german/l2kurz.pdf}}
\item
Kopka, H. \& Daly, P. W. (2003). \textit{Guide to \LaTeX} (4th ed.).
Addison-Wesley.
\item
Oetiker, T., Partl, H., Hyna, I. \& Schlegl, E. (2021, March 9). \textit{The
Not So Short Introduction to \LaTeX2e: Or \LaTeX2e in 139 minutes.} Version 6.4.
...
\begin{itemize}
    ...
    \item
    If texts are generated or revised with the help of AI tools, this must be
    stated in the documentation of AI tools usage (see
    Section~\ref{sec:DocumentingAIUsage}). Content generated by AI tools is not
    one's own work; moreover, the affidavit confirms that the thesis was written
