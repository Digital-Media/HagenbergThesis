\chapter[Handling Literature]{Handling Literature and Other Forms of References}
...
and the table of contents can be specified. This is done by providing an
...
the format of references, determined by the respective field of activity or
guidelines from publishers and universities. This template provides a scheme
that is common in scientific and technical disciplines.%.
...
refer to entries in the \emph{bibliography} (or in multiple bibliographies). The
bibliography is a compilation of all references, typically at the end of the
document. Each citation must have an associated, unique entry in the
...
provides a central command.

\subsection{The \texttt{\textbackslash cite} Command}

For citations in the running text, use the statement
%
\begin{itemize}
...
\end{itemize}
%
\textit{keys} is a comma-separated listing of one or more citation keys to
identify the corresponding entries in the bibliography. \textit{text} can
...
\end{LaTeXCode}
%
    \item For further details, see \cite[chap.~3]{Kopka2003}.
\begin{LaTeXCode}[numbers=none]
For further details, see \cite[chap.~3]{Kopka2003}.
\end{LaTeXCode}
%
    \item The data in \cite[pp.~274--277]{BurgeBurger1999} appears outdated.
\begin{LaTeXCode}[numbers=none]
The data in \cite[pp.~274--277]{BurgeBurger1999} appears outdated.
\end{LaTeXCode}
%
...
%
\begin{itemize}
    \item Something similar can also be found in
    \mcite[chap.~2]{Loimayr2019}[sec.~3.6]{Drake1948}[pp.~5--7]{Eberl1987}.
\begin{LaTeXCode}[numbers=none]
Something similar can also be found in \mcite[chap.~2]{Loimayr2019}[sec.~3.6]{Drake1948}[pp.~5--7]{Eberl1987}.
\end{LaTeXCode}
\end{itemize}
%
For better readability, the output---unlike the regular
\texttt{\textbackslash cite}---includes a \emph{semicolon} (;) as a separator
between the entries. However, sorting the entries (if desired) must be doneö
...
\end{itemize}
%
This also works with \verb!\mcite! and other \verb!cite! macros; the outer
brackets are important here. For example, in the bibliography at the entry
{\backtrackerfalse\parencite{Bezos2022}} the current page (\the\value{page})
should \emph{not} be listed.

\subsection{Common Mistakes}

Several common mistakes tend to occur when working with references, especially
for inexperienced people. However, these can be easily avoided.

\subsubsection{References Outside of the Sentence}

Citations should be placed within or at the end of a sentence (i.e., before the
period), not \emph{outside}:
%
\begin{center}
	\begin{tabular}{rl}
		\textbf{Incorrect:}  & \ldots this is the end of the sentence.
		\cite{Oetiker2021} And now it continues \ldots \\
...
appended directly to the word (like a footnote):
%
\begin{center}
	\begin{tabular}{rl}
		\textbf{Incorrect:}  & \ldots here goes the
...
If an entire paragraph (or more) is quoted from a source, the reference should
...
example also switches to German (see section \ref{sec:language-switching}):%
\footnote{Pay attention to the German quotation marks within the quote, which
...
\subsubsection{Alternative Extensions (Using the Document Option
\texttt{smartquotes})}
...
In rare cases, it happens that one wants (or needs) to cite a source \textbf{A}
that is not available (and thus cannot be read personally) but which is cited in
\emph{another} available source \textbf{B}. In this case, \textbf{A} is called
the \emph{original} or \emph{primary source}, and \textbf{B} is called the
\emph{secondary source}. The following basic rules for such a scenario should be
heeded:
%
\begin{itemize}
    \item \textbf{Secondary sources} should be \textbf{avoided} whenever
    possible.
    \item In order to quote a source in the usual form, one must always have
    \textbf{personally accessed} (read) it!
    \item Only if one can absolutely \textbf{not} obtain the primary source a
    reference via a secondary source is permissible. In this case, primary and
    secondary sources should be \textbf{cited together}, as shown in the example
    below.
    \item \textbf{Important}: Only the available source (B) and not the original
    work is included in the bibliography!
\end{itemize}
%
\textbf{Example}: Suppose one would like to quote a passage from the famous book
\emph{Dialogo} by Galileo Galilei (which is difficult to obtain) found in a more
recent work from 1969. One could accomplish this \eg, with the following
footnote.%
...
\cite[p.~59]{Hemleben1969}.} % All page numbers are fictional!
In the actual bibliography, only the secondary source \cite{Hemleben1969} is
included.
...
institutions and publishers) \textbf{often wrong or syntactically incorrect}!
...
referencing modern multimedia formats. However, this means that the
bibliographic data used in \texttt{biblatex} is no longer fully backward
...
This template's interfaces to \texttt{biblatex} are wrapped mainly in the style
...
\end{LaTeXCode}
%
In the "preamble" (line \ref{tex:references1}),
...
refers to a Biblatex file \nolinkurl{references.bib}, which contains all the
references.

If multiple Biblatex files are used, they can be specified in the same form.

The \verb!\MakeBibliography! statement at the end of the document
...
This template provides the following categories for split bibliographies (see
table~\ref{tab:references-and-entry-types}):%.
...
of a split bibliography, the entries of each category are combined in an
individual section. Elements marked in gray are synonyms for the respective
types above them.}
...
\end{itemize}
%
\emph{Note}: The entry field \texttt{edition} is usually only specified if there
is \emph{more} than one, especially \textbf{not for the 1\textsuperscript{st}
...
between the fields \texttt{venue} to indicate the conference location and
\texttt{location} for the location of the publication (the publisher).
%
\begin{itemize}
\item[]
\begin{GenericCode}[numbers=none]
...
\end{itemize}
%
\emph{Note}: Specifying an issue for \emph{several} months (for example, in the
...
This entry type can be used for any thesis. The exact type is specified via the
...
\end{itemize}
%
The content of the \verb!url={..}! field will be automatically set as URL
...
still be possible to reference them if necessary.
%
\begin{itemize}
\item[]
\begin{GenericCode}[numbers=none]
...
sense to specify the corresponding URL, if available.
%
\begin{itemize}
\item[]
\begin{GenericCode}[numbers=none]
...
documentation does not necessarily have to exist in printed form.
%
\begin{itemize}
\item[]
\begin{GenericCode}[numbers=none]
...
but within \textbf{additional parentheses} \texttt{\{..\}}, so that the
...
unfortunately, no particular entry type in Biblatex. For an \emph{individual}
issue, it is easiest to use the type \texttt{@book}, such as (see also
...
It should be noted that releasing something online does \textbf{not formally
count as a publication}, as these platforms are not publishers. Some of the
...
to determine whether the paper was accepted and published in the end:
%
\begin{itemize}
\item[a)]
\textbf{The work was actually published:} Here the corresponding original
...
\item[b)]
\textbf{The work was \emph{not} published:} If indeed no publication (or an
...
%
\item[] %
In this case, the specification of the link (via \texttt{doi} or alternatively
...
\end{itemize}
%
Instead of \verb!type={audiocd}!, \verb!howpublished={Audio-CD}! could also be used.
...
\noindent
Here is an example of the reference to a DVD edition:
%
\begin{itemize}
\item[]
\begin{GenericCode}[numbers=none]
...
following example (see also \cite{Psycho1960}) the relevant data is given in the
\texttt{note} field:%
...
it is possible to specify a the location of a specific frame by using the
...
\begin{itemize}
    ...
    \verb!\footnote{\url{..}}!.
%
    \item[B.] A printed or audio-visual work (see sections
    ...
    entry type.
%
    \item[C.] It is a reference in the broadest sense, but \emph{exclusively}
    available online, such as a wiki or blog entry. The \emph{online} category
...
the bibliography seems justified. For example, a chasse (German:
"Reliquienschrein") is a shrine in which the relics of one or more saints are
...
\noindent%
\textbf{Note}: Technically, only the \texttt{url} field is required for online
...
which will be used when sorting the bibliography.

\subsection{Tips for Creating Biblatex Files}
\label{sec:tips-on-biblatex}

The following things should be considered for creating correct Biblatex
files.
...
\textbf{1\textsuperscript{st} edition}, the \texttt{edition} field should
\textbf{not} be specified if there is no other edition!
...
last names (\texttt{\textit{ln}}), namely in the form%
...
first and last names cannot be correctly assigned, e.g.
%
\begin{itemize}
\item[]
...
\end{itemize}
%
for a (hypothetical) opus by \emph{Ludwig van Beethoven} and \emph{Jaap ter
Linden}.
...
removes the \texttt{url} field if a DOI is present.
...
the most critical actions related to the bibliography:
%
\begin{itemize}
    \item Check all entries for missing or misinterpreted elements!
    ...
    them if necessary.
    \item Books: Check publisher name and place of publication for completeness,
...
including those for which there is no explicit \verb!\cite{}! call. This is
useful for getting an overview of all references while writing the thesis.
...
\emph{Plagiarism} is representing another's work as one's own creation, in part
or whole, whether consciously or unconsciously. Plagiarism is not a new problem
in higher education. However, it has increased dramatically in recent years due
to the widespread availability of electronic sources and is by no means
considered a trivial offense. Many universities now also use electronic
resources (some of which are not accessible to students) as a countermeasure.
Therefore, one should expect any work handed in to be routinely checked for
plagiarism! If such is discovered only at a later point, this can, in the worst
case, lead to the subsequent (and final) revocation of the academic degree. To
avoid such problems, one should instead act overcautiously and at least observe
the following rules:
%
\begin{itemize}
    \item
    Adopting short text passages is only permitted with correct citation,
    whereby the text citation's scope (beginning and end) must be clear in each
    case.
    \item
    In particular, it is not acceptable to mention a reference only at the
    beginning and subsequently to repeatedly adopt uncited text passages as
    one's own creation.
    \item
    Under no circumstances is the immediate adoption or \emph{paraphrasing} of
    longer text passages tolerable, whether with or without citation. Passages
    expressed indirectly or translated from another language must also be cited
    properly!
\end{itemize}
%
In case of doubt, more detailed rules can be found in any good book on
scientific writing. To be safe, asking one's supervisor is always advisable.
