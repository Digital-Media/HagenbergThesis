...
\footnote{This setup builds on the PDF management of the \latex kernel (package
\texttt{pdfmanagement}, or \texttt{pdfmanagement-testphase} with older
distributions) and therefore requires a current \latex distribution.}
If no PDF/A output is needed, this statement can simply be removed. The fonts
(Latin Modern with T1 encoding, see Section~\ref{sec:fonts}) are not affected.
...
PDF/A-compliant but this does not imply that it actually \emph{is}.
...
file is sufficient. The affidavit is then either signed digitally in the
...
high-quality printer (typically a \emph{laser printer}); printouts made with
simple inkjet printers are usually less suitable. The paper used should also be of good quality (woodfree)
