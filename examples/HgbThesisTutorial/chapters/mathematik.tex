...
(s.\ Quellcode in Prog.~\ref{prog:AlgExample}).


%%--------------------------------------------------------------------

\begin{algorithm}
\caption{Bikubische Interpolation in 2D. Die in den Zeilen \ref{alg:wcub1} und
	...
	\For{$j \gets 0, \ldots, 3$} \Comment{iterate over 4 lines}
		...
		\For{$i \gets 0, \ldots, 3$} \Comment{iterate over 4 columns}
			...
			\State $p \gets p + I(u,v) \cdot \Call{Cubic1D}{x - u}$ % w_{\mathrm{cub}}(x - u )$
					\label{alg:wcub1}
		\EndFor		
		\StateNN[2]{Sometimes it is useful to insert a longer, \emph{unnumbered} explanation that wraps 
		over multiple lines with proper indentation. This can be done with the (non-standard) command 
		\texttt{{\bs}StateNN[]\{..\}}. There is also a \texttt{{\bs}StateL\{..\}} command for regular 
		(i.e., \emph{numbered}) multi-line statements.}
...
\Procedure{Cubic1D}{$x$} \Comment{piecewise cubic polynomial}
	\State $z \gets 0$
		\If{$|x| < 1$}
