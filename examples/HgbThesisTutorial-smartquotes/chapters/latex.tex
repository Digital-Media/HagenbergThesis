...
Mit Anführungszeichen wird aus Gewohnheit recht
nachlässig umgegangen und auch dabei sind die Unterschiede zwischen Deutsch
und Englisch zu beachten.


\subsubsection{Variante 1: Hochkommas mit der \latex-Standardeinstellung}

Mit der Standardeinstellung von \latex\ (\emph{ohne} Verwendung der Option \texttt{\texttt{smartquotes}})
muss die Eingabe von vorderen und hinteren Hochkommas exakt erfolgen und führt daher
häufig zu Fehlern.
Hier die richtige \latex-Notation für beide Sprachen:
%
\begin{quote}
...
\end{quote}
%
Manche Editoren (\ua\ TeXnicCenter) kann man so einstellen, dass
bei der Eingabe eines einfachen Hochkommas die entsprechenden Zeichenfolge 
automatisch (kontextabhängig) eingesetzt wird.
...
\subsubsection{Variante 2: Hochkommas mit der Option \texttt{\bfseries smartquotes} (experimentell!)}

Diese Version der Vorlage verwendet ein \textbf{experimentelles Setup},
das durch die Dokumentenoption \enquote{\texttt{smartquotes}} aktiviert wird
und auf dem \texttt{csquotes}-Paket%
\footnote{\url{https://ctan.org/pkg/csquotes}}
aufbaut.
Die korrekte Verwendung von Hochkommas vereinfacht sich damit deutlich, weil
abhängig von der aktuellen Spracheinstellung und der Position des Hochkommas
das jeweils richtige Zeichen eingesetzt wird.
Es genügt hier die Verwendung eines doppelten (geraden) Hochkommas \texttt{\char34}, wie \zB
%
\begin{quote}
...
\end{quote}
%
Dabei ist zu beachten, dass die traditionelle Eingabe von Hochkommas (s.\ Variante~1, oben) hier nicht
mehr zur Verfügung steht. Die gemischte Verwendung von Variante~1 und Variante~2 ist somit
nicht möglich! Es sind in diesem Fall auch alle weiteren "shorthands" des 
\texttt{babel}-Pakets%
\footnote{\url{https://ctan.org/pkg/babel}}
(wie \zB\ \verb!"a!, \verb!"o!, \verb!"u!) permanent \emph{deaktiviert} und diese können
derzeit auch lokal nicht reaktiviert werden.%
\footnote{Die hier eingestellte Verwendung des \texttt{\char34} Zeichens als 
beidseitiges "outer quote" Zeichen gilt -- \va\ in Kombination mit der deutschen Sprache --
als "gefährlich", weil \texttt{babel} seinerseits das gerade Hochkomma für zahlreiche
\emph{shorthand}-Makros nutzt. Das nehmen wir mutig in Kauf, allerdings sind die \texttt{babel}-shorthands im aktuellen Setup
generell deaktiviert um Schwierigkeiten zu vermeiden.}

\paragraph{Verwendung des \texttt{csquotes}-Pakets:}
Das (mit dieser Einstellung automatisch geladene)
...
\end{itemize}
%
Eine weiteres praktisches Feature bietet das Makro \verb!\foreignquote!, mit dem man sehr einfach
...
\footnote{Diese Möglichkeit ist alleridngs in Kombination mit der Dokumentenoption
\texttt{smartquotes} nicht verfügbar, weil dadurch alle derartigen \texttt{babel} "shorthands"
deaktiviert sind.}
