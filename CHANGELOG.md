...
- 2026/09/14: [#229](https://github.com/Digital-Media/HagenbergThesis/issues/229): Tutorials (`HgbThesisTutorialDE`/`HgbThesisTutorialEN`): in the Java example (program 4.2), the math in a comment is typeset with the option `mathescape=true` in the comment style (previously in an escape, without the comment color), the class name in the last comment is no longer escaped, and the German version no longer aligns a comment with tabs; the closing `\\` of the table source (program 4.1) are aligned; five overfull boxes are avoided by rewording or hyphenation hints.
...
- 2026/09/14: [#227](https://github.com/Digital-Media/HagenbergThesis/issues/227): In the detailed documentation of AI tools usage, "Keine"/"None" is printed below the heading of further use if no further use is documented with `\aifurtheruse` (previously, the heading stood alone). `\aifurtheruse` is used once per further use; several entries are printed as a bulleted list (previously as separate, indented paragraphs).
...
- 2026/09/14: [#227](https://github.com/Digital-Media/HagenbergThesis/issues/227): Corrected the German texts of the documentation of AI tools usage (taken over from the FH OÖ templates): the date format in the source reference below the detailed form ("22. Mai 2023" instead of "Mai 22, 2023") and no comma before "etc." (e.g., "Zusammenfassungen etc."); the German texts of the detailed form use the gender star as the rest of the package ("dem\*der Autor\*in bzw. den Autor\*innen" instead of "dem/den Autor(en)", "Autor\*innen" instead of "Autor:innen").
