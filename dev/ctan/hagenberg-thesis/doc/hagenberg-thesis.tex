...
command but provides controlled indentation inside nested constructs.
...
between individual statements of an algorithm (the standard spacing commands like 
\verb!\smallskip! \etc\ have no effect between statements):%
\footnote{Note that the standard spacing commands work \emph{between} \texttt{procedure} 
and \texttt{function} blocks in the usual way.}
\begin{itemize}
...
\end{itemize}
They are supposed to be used inside (\ie, at the end of) statements, for example:
%
\begin{LaTeXCode}[numbers=none]
...
\subsection{Abbreviations, Acronyms and Nomenclature (\texttt{hgbacro.sty})}

\texttt{hgbacro} is a wrapper for the \texttt{acro}%
\footnote{\url{https://ctan.org/pkg/acro}}
package. It provides a simple and pre-configured setup to typeset acronyms or
nomenclature.

Acronyms should be defined in a file called \texttt{acronyms.tex} in the
root directory. By using the \verb|\ac{...}| command, acronyms can be referred in
the running text. The extended description is used for the first appearance, whereas the
acronym is printed for any subsequent use.

To display a list of acronyms, use the \verb|\PrintAcronyms| command.

For additional options, please refer to the \texttt{acro} manual.
