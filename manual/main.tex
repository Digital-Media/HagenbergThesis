...
\begin{itemize}
	...
	\item \texttt{smartquotes} (= \texttt{smartquotes=true}):
		smart quotes replacement (\verb!"!...\verb!"! $\rightarrow$ ``...'');
		use \texttt{smartquotes=false} to disable explicitly,
	\item \texttt{apa} (= \texttt{apa=true}):
		use APA bibliography style instead of \texttt{numeric-comp};
		default is \texttt{apa=false},
	\item \texttt{noUpdateCheck} (= \texttt{noUpdateCheck=true}):
		suppress check of package release date.
\end{itemize}
...
\noindent\textbf{\texttt{hgbthesis}}:
	\begin{itemize}
		...
		\item \texttt{theme=}\{\textit{themeID}\}:
			cover page theme style (default: \texttt{default}),
		\item \texttt{proposal} (= \texttt{proposal=true}): flag to indicate proposal documents.
	\end{itemize}
...
\noindent\textbf{\texttt{hgbarticle}}:
	\begin{itemize}
		\item \texttt{twocolumn} (= \texttt{twocolumn=true}): two-column page layout,
		\item \texttt{review} (= \texttt{review=true}): review mode with line numbers.
	\end{itemize}
