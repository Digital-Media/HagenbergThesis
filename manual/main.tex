...
\begin{itemize}
	...
	\item \verb!\aifurtheruse{<text>}!: further use of AI tools that fits none of the activities
		(optional). Use one \verb!\aifurtheruse! per use: a single entry is printed as a paragraph,
		several entries as a bulleted list; without any entry, \emph{None} or \emph{Keine} is printed
		below the heading.
\end{itemize}
%
...
\end{LaTeXCode}
%
The sample documents contain an example line for every activity, most of them commented out. Unlike
the template, the form contains no instructions to the authors (\eg, to name the AI tools used);
these are given as comments in the sample documents instead.
